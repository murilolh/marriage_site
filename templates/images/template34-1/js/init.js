...
var Pixastic = (function() {
	...
	function onready(handler) {
		...
		_script.setAttributeNode(document.createAttribute("defer"));
		_script.id = "__onload_ie_pixastic__"
...
function delega() {
	...
	if ($("#custom-foto25 div").length != 0) {

		if (typeof delegado != 'undefined') clearTimeout(delegado);
		run();

	}else{

		delegado = setTimeout(function() { delega() }, 500);
...
var run = function(){
	...
	img.onload = function() {
		...
		img.height = $("#header").width();
		Pixastic.process(img, "blurfast", {amount:50}, function() {
	...
	img.id = "imgBlr";
	img.width = ImgBlur.width();
	img.height = 261;
	img.src = $("#custom-foto25 div").css("backgroundImage").replace("url(","").replace(")","").replace("180","100").replace("180","20").replace(/"/g,"");
	ImgBlur.css({
	...
	$("#xx").css({
		...
		"top":"0",
		"zIndex":"1",
		"background":"url(/assets/templates/commun/bg37-blur.png)",
		"background-color": "black"
	}).width(img.width).height(261);
};
