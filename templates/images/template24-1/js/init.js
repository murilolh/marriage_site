...
$(document).ready(function(){
	...
	var fxResize = function() {
		...
		$measurer = $("<div>").css({
		    ...
		    top: '100%',
		    whiteSpace: 'nowrap'
		}).attr('id', 'measurer').attr("class","measure");
		$measurer.append($("#header h1").text()), escape = 0;
		$measurer.appendTo("body");
	
		while ($measurer.width() <= 700) {
		    $measurer.css('font-size', '+=1px');
		    escape++;
		    if (escape == 100) break;
		}
		
			if ($measurer.height() >= 70){
				while ($measurer.height() >= 70) {
				    $measurer.css('font-size', '-=1px');
				    escape++;
				    if (escape == 100) break;
			    }
		...
		var msf = $measurer.css('font-size');
		msf = msf.replace("px","")*1;
		
		if (msf > 63) msf = 63;
		
		$("#header h1").css("font-size", msf+"px").show();
